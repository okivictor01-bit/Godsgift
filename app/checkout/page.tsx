...
export default function Checkout() {
  ...
  const saveOrderToDatabase = async (reference: string) => {
    try {
      alert('Step 1: Starting to save order...');
      ...
      alert('✅ Order saved! Saving items...');
      ...
      try {
        const handler = PaystackPop.setup({
          ...
          callback: function(response: any) {
            alert('💰 Payment Successful! Saving to database...');
            saveOrderToDatabase(response.reference).finally(() => {
