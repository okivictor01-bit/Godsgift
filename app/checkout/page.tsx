  ...
  const saveOrderToDatabase = async (reference: string) => {
    try {
      ...
      for (const item of cart) {
        ...
        if (itemError) {
          console.warn('Item save warning:', itemError.message);
        }
      ...
      // THIS IS THE IMPORTANT PART: It will show the exact error on your screen
      ...
      try {
        const handler = PaystackPop.setup({
          ...
          callback: function(response: any) {
            // Payment worked! Now save to database
            saveOrderToDatabase(response.reference).then((success) => {
              if (success) {
                alert('✅ Payment successful and order saved!');
              } else {
                alert('⚠️ Payment worked, but database save failed. Check the error message above.');
              }
            }).finally(() => {
